...
const interface = ustack.interface;
...
var global_iface: *interface.NetworkInterface = undefined;
...
pub fn main() !void {
    ...
    if (args.len < 5) {
        std.debug.print("Usage: {s} <interface> <mode> <ip_address/cidr> <driver> [target_ip]\n", .{args[0]});
        std.debug.print("  mode: server | benchmark\n", .{});
        std.debug.print("  driver: packet | xdp | tap\n", .{});
        return;
    }
    ...
    my_ev_io_init(&io_watcher, libev_driver_cb, global_iface.getFd(), c.EV_READ);
...
fn libev_driver_cb(loop: ?*anyopaque, watcher: *c.ev_io, revents: i32) callconv(.C) void {
    _ = loop;
    _ = watcher;
    _ = revents;
...
fn libev_safety_cb(loop: ?*anyopaque, watcher: *c.ev_timer, revents: i32) callconv(.C) void {
    ...
    _ = watcher;
    _ = revents;
    std.debug.print("Safety timeout reached. Aborting.\n", .{});
    std.process.exit(1);
}
...
// Benchmark Logic (Copied for completeness)
