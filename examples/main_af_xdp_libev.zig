...
pub fn main() !void {
    ...
    // Increase stack limits for high concurrency
    // ustack default might be small, but here we allocate dynamic structures.
...
const HttpClient = struct {
    ...
    pub fn init(s: *stack.Stack, allocator: std.mem.Allocator, mux: *EventMultiplexer) !*HttpClient {
        ...
        self.* = .{
            ...
            .allocator = allocator,
            .app_entry = .{
                .wait_entry = waiter.Entry.init(mux, EventMultiplexer.upcall),
                .ctx = .{ .client = self },
            },
        };
    ...
    fn finish(self: *HttpClient, success: bool) void {
        self.state = .closed;
        self.ep.close();
        // Don't free wq here if owned by ep, but for active open we created it.
        // wait, we passed wq to newEndpoint.
        // TCPEndpoint init takes it. Does it own it?
        // Check tcp.zig init: new_ep.owns_waiter_queue = false (default) unless explicitly set?
        // Actually, init() returns struct by value.
        // The one in stack.zig newEndpoint:
        // var ep = try TCPEndpoint.init(stack, wq);
        // It does NOT set owns_waiter_queue = true.
        // So we MUST free it.
        
        self.ep.close(); // Double close safe?
        // Wait, ep.close() calls vtable.close -> close_external -> decRef -> destroy
        // destroy calls allocator.destroy(waiter_queue) IF owns_waiter_queue.
        // If not owned, we destroy it.
        
        // We have it in self.wq.
...
const Connection = struct {
    ...
    fn close(self: *Connection) void {
        ...
        // wq owned by endpoint, do not free? NO, for accepted connections, TCPEndpoint owns WQ.
        // See previous debugging.
        alloc.destroy(self);
    }
};
